...
Current active plan: `PLAN.md` is now the full card clue audit plan. It is split into 19 measurable action items, and each item must keep concrete acceptance evidence before it is marked complete. `CLUE_AUDIT.md` records the current full-card review. Review all shipped puzzle cards and ensure emoji clues do not include the answer's direct emoji or an overly literal answer substitute. Examples: `Whale` must not use `🐋`, `Fox` must not use `🦊`, `Elephant` must not use `🐘`, `Giraffe` must not use `🦒`, and direct fruit/vegetable answers should not use their matching produce emoji.

Future card additions must update the answer emoji banlist in `src/data/answerEmojiBanlist.ts` whenever the answer has a direct emoji representation. Run the clue-audit tests in `src/lib/clue-audit.test.ts` before committing new or changed cards.
