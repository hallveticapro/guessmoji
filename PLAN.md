...
| 2.3 Stale last-category link removal | fixed and verified | Source removes synthetic fallback and clears invalid slug; `src/components/categories/last-category.test.ts` verifies valid, invalid, and empty saved slugs; `src/components/categories/LastCategoryLink.test.tsx` jsdom-renders valid and stale localStorage cases; browser smoke verifies valid `/play/pixar` link. |
...
| 3.2 ResizeObserver fallback | fixed and verified | `EmojiClue` checks for `ResizeObserver`; `src/components/game/emoji-fit.test.ts` verifies missing and available support; `src/components/game/EmojiClue.test.tsx` renders with `window.ResizeObserver` removed and no throw; mobile browser smoke confirms one-line nowrap shrink-to-fit. |
