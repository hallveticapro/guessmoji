...
Task 7: fix round 3/5 open after 0ea3718 — removed the complete Robots addition block after `robots-space-drone` failed delta2 blind review, retained only Jobs/Idioms (20 additions), corrected the live PBS Lemonade Stand source, added per-card retained source records, and documented the lexicalized-metaphor carve-out; C total 220, fresh delta blind review pending.
Task 7: fix round 4/5 open after 6d27fb7 — removed answer names from the player-visible Literal Phrases description while retaining the non-player carve-out documentation, repaired Camping S'mores to `🙋➕1️⃣😋` wordplay and Zion to red-rock/cable-route/river-canyon clues, and added focused regressions; full validation 120/120, fresh two-card plus metadata blind review pending.
Task 7: complete (b731413) — final source/rules acceptance is approved in `task-7-round5-source-acceptance.md`, and the final acceptance blind packet `task-7-round5-acceptance-blind.json` passes the retained S'mores/Zion cards plus Literal Phrases metadata; Partition C remains 220 cards with only complete Jobs/Idioms additions, and focused/full tests, typecheck, lint, build, and diff-check are green.
Task 8: complete (5d5a28c) — corrected the Dumbledore alchemy fact, separated Golden Snitch and Quidditch clues, and adjudicated the revised component/context exceptions; the fresh staged delta review passes 3/3 and, with the carried-forward evidence, closes all 20/20 Harry Potter cards; independent source/rules review approves the complete block.
Task 9: complete — final catalog counts, dynamic packet balancing, ten-card session/history invariants, Random Mix metadata hiding, route aggregation, and documentation synchronization are implemented; 15 files/133 tests, typecheck, lint, build, Docker build, diff-check, manifest validation, and final packet coverage are green. Final packets cover 730/730 opaque IDs across 60/60 source categories with 250/240/240 card splits and blind/full/hint parity.
